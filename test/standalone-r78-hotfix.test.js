...
test('manager backend revision enables the documented safe input fallback', () => {
  ...
  assert.match(optiscaler, /packageId: '0\.7\.7-dlss5mgr29'/);
  ...
  assert.match(optiscaler, /\['Inputs', 'EnableXeSSInputs', 'true'\]/);
  assert.match(optiscaler, /\['Inputs', 'EnableFsr2Inputs', 'true'\]/);
  assert.match(optiscaler, /\['Inputs', 'EnableFsr3Inputs', 'true'\]/);
  assert.match(optiscaler, /\['Inputs', 'EnableFfxInputs', 'true'\]/);
});
...
test('library updates a managed backend in place instead of restore then install', () => {
  ...
  const optiscaler = read('standalone/core/optiscaler.js');
  assert.match(compat, /MANAGER_BACKEND_ID = '0\.7\.7-dlss5mgr29'/);
